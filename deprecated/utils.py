...
class SamplePatch(object):
    ...
    def __call__(self, img):
        ...
        assert (
            self.size_max <= h
        ), "size_max has to be smaller than or equal to image height."
...
class Dropper(object):
    def __init__(self, drop=0.0, drop2d=False):
        self.drop = drop
        self.drop2d = drop2d
...
class RandomPool(object):
    ...
    def __call__(self, img, pad_val=None):
        pad_val = pad_val if pad_val is not None else random.random() * 2.0 - 1.0
        # Double uniform sample different distribution
        kernel_size = random.randint(
            self.kernel_min, random.randint(self.kernel_min, self.kernel_max)
        )
...
def grad_drop(grad, drop=0.0, drop2d=True):
    """Dropout gradient."""
    with torch.no_grad():
        if not drop2d:
